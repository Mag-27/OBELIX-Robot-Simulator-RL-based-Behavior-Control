"""PPO trainer for OBELIX — matches lecture pseudocode exactly, extended with:
  ...
  • Mode-conditioned soft logit bias (learnable parameter)
...
class RewardShaper:
    # SEARCH
    R_SCAN_TURN    =  0.05   # encourage scanning, but not overwhelmingly
    R_BLIND_FW     = -0.05   # mild penalty for blind forward motion
...
class PolicyNetwork(nn.Module):
    def __init__(
        ...
        self.mode_logit_bias = nn.Parameter(
            torch.empty(N_MODES, n_actions, dtype=torch.float32).uniform_(-0.1, 0.1)
        )
...
class PPO:
    def __init__(
        ...
        self.args                      = args
        self.eval_every                = getattr(args, 'eval_every', 10)
        self.curriculum_interval       = getattr(args, 'curriculum_interval', 10)
        self.next_eval_checkpoint      = self.eval_every
    ...
    def trainAgent(self):
        ...
        totalEps   = 0
        fill_count = 0
        use_walls  = False   # start without walls; toggle every curriculum interval
        ...
        while True:
            ...
            while totalEps >= self.next_eval_checkpoint:
                print(f"Switching to rendered evaluation at episode {self.next_eval_checkpoint}", flush=True)
                eval_mean, eval_std = self.evaluateAgent(render=True, num_episodes=1)
                print(f"  [eval milestone] rendered eval mean={eval_mean:.2f} std={eval_std:.2f}", flush=True)
                self.next_eval_checkpoint += self.eval_every

            # Toggle wall_obstacles every curriculum interval
            ...
            if new_use_walls != use_walls:
                ...
                print(f"Switching to {phase} phase at episode {totalEps}", flush=True)
        ...
        with torch.no_grad():
            for e in range(num_episodes):
                ...
                for c in count():
                    ...
                    raw2, r, done = env.step(ACTIONS[a], render=eval_render)
...
def main():
    ...
    ap.add_argument("--fw_floor",              type=float, default=0.0)
    ap.add_argument("--eps_start",             type=float, default=0.10)
    ap.add_argument("--eps_end",               type=float, default=0.01)
    ap.add_argument("--fw_rollout_rate",       type=float, default=0.0)
    ap.add_argument("--fw_rollout_rate_end",   type=float, default=0.0)
    ap.add_argument("--search_extra_eps",      type=float, default=0.0)
    ap.add_argument("--unstuck_fw_penalty",    type=float, default=0.0)
    ap.add_argument("--rotation_penalty",      type=float, default=0.002)
